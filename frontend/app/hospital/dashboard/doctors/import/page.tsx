...
export default function ImportDoctorsPage() {
  ...
  const copyCredentials = (doctor: any) => {
    const text = `Name: ${doctor.name}\nEmail: ${doctor.email}\nPassword: ${doctor.password}`
    ...
    <div className="space-y-6">
      ...
      {importResult && (
        <div className="space-y-6">
          ...
          {/* Imported Doctors */}
          {importResult.imported_doctors.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {importResult.imported_doctors.map((doctor, index) => (
                      <tr key={index} className="border-b border-gray-100">
                        <td className="py-2 font-medium">{doctor.name}</td>
                        <td className="py-2 font-mono text-blue-600">{doctor.email}</td>
                        <td className="py-2 font-mono">
                          {showPasswords ? doctor.password : '••••••••'}
                        </td>
                        <td className="py-2">{doctor.specialization}</td>
                        <td className="py-2">
                          <button
                            onClick={() => copyCredentials(doctor)}
