...
export default function PricingCard({
  ...
  return (
    <motion.div
      ...
      className={`relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 ${popular ? 'ring-2 ring-blue-500 scale-105' : ''
        }`}
    >
      {popular && (
      ...
      <div className="p-8">
          ...
          <ul className="space-y-3">
            {features.map((feature, featureIndex) => (
              <li key={featureIndex} className="flex items-start">
                <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mr-3 mt-0.5 transition-colors ${feature.included
                ...
                <span className={`text-sm transition-colors ${feature.included ? 'text-gray-900' : 'text-gray-400'
                  }`}>
                  {feature.name}
                </span>
              </li>
        ...
        <InteractiveHoverButton
          className={cn("w-full py-3", buttonStyle)}
        >
          {buttonText}
        </InteractiveHoverButton>
